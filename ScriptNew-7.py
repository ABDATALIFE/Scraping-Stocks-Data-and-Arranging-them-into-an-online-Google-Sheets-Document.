    ...
    for i,sheet in enumerate(sheets):
        ...
        if data:
            ...
            with open(f'NewStocks/{sheet_name}.csv', 'w', newline='') as f:
                ...
                print("Working in progress"+"  "+ f"{i}" )

#%%
csv_files = glob.glob('NewStocks/*.csv')
stocks = [os.path.splitext(os.path.basename(file))[0] for file in csv_files]
stocks_rank = {}


#%%xx
for i,stock_file in enumerate(stocks):
    full_path = os.path.join('NewStocks', f'{stock_file}.csv')
    stock = pd.read_csv(full_path, names = range(16))
    stock = stock.iloc[:,:6]
    ...
    model_filename = os.path.join('models', f'{stock_file}.h5')
    ...
    print("Today's Date: {}, Daily Growth Rate: {:.2f}%, Daily Stock Return: ${:.2f}".format(today_date,
                                                                                           daily_growth_rate_today * 100,
                                                                                             stock_return_today))
    print(f'{model_filename} model done')  
  #%%
